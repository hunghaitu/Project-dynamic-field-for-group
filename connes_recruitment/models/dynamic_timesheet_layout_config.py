...
class DynamicTimesheetLayoutConfig(models.Model):
    ...
    cluster_id = fields.Many2one("cennos.cluster", string="Cluster", required=True)
    ...
    def _check_and_generate_group_view(self, group):
        ...
        xml_id = f"connes_recruitment.dynamic_timesheet_view_project_group_{group.id}"
        view_domain = [("model", "=", "project.project"), ("key", "=", xml_id)]
        custom_view = self.env["ir.ui.view"].sudo().search(view_domain, limit=1)
        parent_view = self.env.ref("connes_recruitment.view_project_project_form_inherited", raise_if_not_found=False)
